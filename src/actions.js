...
const setLanguage = makeActionCreator(
    actionType.GENERAL_SET_LANGUAGE,
    'language'
)
export const setUser = makeActionCreator(actionType.GENERAL_SET_USER, 'user')
export const setLoading = makeActionCreator(
    actionType.GENERAL_SET_LOADING,
    'loading'
)
const setInitialized = makeActionCreator(
    actionType.GENERAL_SET_INITIALIZED,
    'initialized'
)
...
export function setUILanguage(lang, initial) {
    return (dispatch, getState) => {
        ...
        fetch(
            `https://api.bnstree.com/languages/${lang}${initial
                ? '?initial=true'
                : ''}`,
            {
                method: 'post',
