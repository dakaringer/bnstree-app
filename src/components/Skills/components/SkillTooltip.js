...
const SkillTooltip = (props) => {
    ...
    comparisonInfo.forEach((i, type) => {
        ...
        infoList[type] = <div className='info-box'>
    ...
    moveData.get('tags', List()).forEach(tag => {
        tags.push(
            <div className='skillTag' key={tag}>{t(`TAG_${tag}`)}</div>
        )
    })
    comparisonData.get('tags', List()).forEach(tag => {
        if (!moveData.get('tags', List()).includes(tag)) {
            disabledTags.push(
                <div className='skillTag disabled' key={tag}>{t(`TAG_${tag}`)}</div>
            )
        }
    })

    let moveNumber = moveData.get('move')
    let classification = moveNumber > 3 ? t('moveTypeHM', {move: moveNumber-3}) : t('moveType', {move: moveNumber})

    return (
        <div className='skill-tooltip'>
            <div className='tooltip-header'>
                <h2 className='skill-name'>{moveData.get('name')} <small>{classification}</small></h2>
                <p className='skill-focus'>{focusTxt}</p>
            </div>
            <div className='tooltip-main-block'>
                <img className='tooltip-icon' alt={moveData.get('skillId')} src={`https://static.bnstree.com/images/skill/${moveData.get('icon')}`}/>
                <div>
