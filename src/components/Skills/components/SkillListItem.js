...
const SkillListItem = props => {
    ...
    skillData.get('moves', List()).forEach((move, i) => {
        ...
        skillData.get('moves', List()).forEach((hmMove, j) => {
            if (hmMove.get('move') === moveNumber + 3) {
                let hmTooltip = (
                    <SkillTooltip
                        moveData={hmMove}
                        comparisonData={skillData.getIn(
                            ['moves', moveIndex],
                            hmMove
                        )}
                    />
                ...
                hm = (
                    <Tooltip
                        ...
                        <div
                            className={`hmButton ${currentMove ===
                            moveNumber + 3
                                ? 'active'
                                : ''}`}
                            onClick={() =>
        ...
        moves.push(
            <div
                key={moveNumber}
                className={`skill-move-wrapper ${move
                    .get('filter', List())
                    .includes(filter)
                    ? 'filtered'
                    : ''}`}>
    ...
    return (
        <div className="skill-list-item">
            ...
            <div>
                ...
                <div
                    className={`skill-moves ${moves.length === 1
                        ? 'single'
                        : ''}`}>
                    {moves}
                </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(
    translate('skills')(SkillListItem)
)
